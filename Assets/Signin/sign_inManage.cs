...
public class sign_inManage : UnitySingleton<sign_inManage>
{
    ...
    private void Awake()
    {
        ...
        back_reward.onClick.AddListener(() => this.gameObject.SetActive(false));
        IsOpenSignPanel();
    ...
    public void SignIn()//签到
    {
        ...
        GameManager.Instance.AddBlonecount += 2;
        GameManager.Instance.gameDate.addbloneCount += 2;
    }
    public void IsOpenSignPanel() //判断是否要打开签到页面
    {
        // 把字符串类型日期转换为日期类型
        // SeverNow = Convert.ToDateTime(Nowtime); //本次登录的时间
        SignInDate.NowSignin_time = DateTime.Now.ToString();
